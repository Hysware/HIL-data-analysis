...
// Last-Updated: 六 1月 15 07:58:49 2022 (+0800)
//           By: Hongyi Wu(吴鸿毅)
//     Update #: 145
...
#define ENERGYMAX 2048  //keV
#define FILENAME_M4B  "co59.m4b"
#elif DATAFORMAT == DATAFORMAT_ROOT//如果采用 root 格式数据，则修改以下定义
#define FILENAME_ROOT "1553-1607.root"
#define ROOTTH2TYPE  TH2I  //可选择 TH2I TH2F TH2D
#define ROOTTH2NAME  "mat_all"
#endif


#define NOEXPONENT //注释本行，则一维直方图中Y轴采用科学计数法；开启本行，则采用非科学计数法
...
void gaddshowfile(int f1, int f2=-1, int f3=-1, int f4=-1, int f5=-1, int f6=-1);
void gandshowfile(int f1, int f2=-1, int f3=-1, int f4=-1, int f5=-1, int f6=-1);
void gsubshowfile(int f1, int f2=-1, int f3=-1, int f4=-1, int f5=-1, int f6=-1);
void gshowfile(int f);
...
void gshow()
{
  ...
  hg->SetTitle(sht);
#ifdef NOEXPONENT
  hg->GetYaxis()->SetNoExponent(kTRUE);
#endif
  for(auto is=peakbin.begin();is!=peakbin.end();is++)
    {
      htmp->Reset();
...
void gaddshowfile(int f1, int f2, int f3, int f4, int f5, int f6)
{
  TH1 *h1tmp[6];
  int fnum[6];
  TFile *filer[6];
  ...
  for(int i=0;i<6;i++)
    {
      ...
      if(fnum[i] != -1)
	{
	  filer[i] = new TFile(TString::Format("%d.root",fnum[i]).Data(),"READ");
	  if(!filer[i]->IsOpen()) { std::cout<<"Can't open root file: "<<fnum[i]<<".root"<<std::endl; return;}
	  h1tmp[i] = (TH1D*)filer[i]->Get(TString::Format("hg%d",fnum[i]).Data());
	}
    }
 
  gaddshow(h1tmp[0],h1tmp[1],h1tmp[2],h1tmp[3],h1tmp[4],h1tmp[5]);
}

void gandshowfile(int f1, int f2, int f3, int f4, int f5, int f6)
{
  TH1 *h1tmp[6];
  int fnum[6];
  TFile *filer[6];
  ...
  gandshow(h1tmp[0],h1tmp[1],h1tmp[2],h1tmp[3],h1tmp[4],h1tmp[5]);
}

void gsubshowfile(int f1, int f2, int f3, int f4, int f5, int f6)
{
  TH1 *h1tmp[6];
  int fnum[6];
  TFile *filer[6];
  ...
  gsubshow(h1tmp[0],h1tmp[1],h1tmp[2],h1tmp[3],h1tmp[4],h1tmp[5]);
}
...
void gshowfile(int f)
{
  ...
  TFile *filer = new TFile(TString::Format("%d.root",f).Data(),"READ");
  if(!filer->IsOpen()) { std::cout<<"Can't open root file: "<<f<<".root"<<std::endl; return;}
  h1tmp = (TH1D*)filer->Get(TString::Format("hg%d",f).Data());
