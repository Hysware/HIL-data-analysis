...
void makeggmat(TString rootfile = "/mnt/hgfs/Shared/11B+196Pt@60MeV-202111/codes/4110_6481.root",
TString ggfile="/mnt/hgfs/Shared/11B+196Pt@60MeV-202111/codes/4110_6481_meiyoubg.root")
{
  int nebin=8192;//number of bin for TH2I
  double emax=2048;//maximum energy .
  ...
  auto hgg=(TH2I*)gROOT->FindObject("mat_all"); //gg matrix
  ...
  // TString satcut="1";
  // TString sgtree1=Form("ae>>ae(%i,0,%f)",nebin,emax);//for precise peak value;
  ...
  auto hbx=sx->Background(hx,15,"same"); //background of Tpj................
  ...
  for(int i=0;i<hgg->GetNbinsX();i++) {
    for(int j=0;j<hgg->GetNbinsY();j++) {
      ...
      Bij=(Pi*Pj-pi*pj)/T;
      // if(Pj>100&&Pi>100) cout <<pi<< " " << pj << " " << Pi << " " << Pj<< " "<< Bij <<" "<< T << endl;
